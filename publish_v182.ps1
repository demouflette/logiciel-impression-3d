...
$setupPath = "D:\projet_3d\logiciel d'impression 3d\Installer\Crea_Cout_3D_Setup_1.8.2.0.exe"
...
Write-Host "1. Creation release v1.8.2..."
$releaseObj = @{
    tag_name         = "v1.8.2"
    target_commitish = "master"
    name             = "v1.8.2 - Correction affichage quantite serie"
    body             = "### Correction`n`n- Correction : le champ Quantite (serie) etait masque derriere le panneau Parametres d'impression dans l'onglet Bambu Lab"
    draft            = $true
    prerelease       = $false
}
...
$a2 = Invoke-RestMethod -Uri "https://uploads.github.com/repos/$owner/$repo/releases/$($release.id)/assets?name=Crea_Cout_3D_Setup_1.8.2.0.exe" -Method POST -Headers $uploadHeaders -Body $setupBytes
