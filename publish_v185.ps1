...
$setupPath = "D:\projet_3d\logiciel d'impression 3d\Installer\Crea_Cout_3D_Setup_1.8.5.0.exe"
...
Write-Host "1. Creation release v1.8.5..."
$releaseObj = @{
    tag_name         = "v1.8.5"
    target_commitish = "master"
    name             = "v1.8.5 - Correction affichage AMS (DPI)"
    body             = "### Correction`n`n- Correction definitive : hauteur du panneau Parametres d'impression recalculee apres le DPI scaling (evenement Load). Le champ Nombre de couleurs est maintenant toujours visible apres avoir coche Utiliser AMS"
    draft            = $true
    prerelease       = $false
}
...
$a2 = Invoke-RestMethod -Uri "https://uploads.github.com/repos/$owner/$repo/releases/$($release.id)/assets?name=Crea_Cout_3D_Setup_1.8.5.0.exe" -Method POST -Headers $uploadHeaders -Body $setupBytes
