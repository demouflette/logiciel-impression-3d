...
$setupPath = "D:\projet_3d\logiciel d'impression 3d\Installer\Crea_Cout_3D_Setup_1.8.3.0.exe"
...
Write-Host "1. Creation release v1.8.3..."
$releaseObj = @{
    tag_name         = "v1.8.3"
    target_commitish = "master"
    name             = "v1.8.3 - Correction AMS nombre de couleurs masque"
    body             = "### Correction`n`n- Correction : apres avoir coche Utiliser AMS, le champ Nombre de couleurs etait masque (controles en dehors des limites du GroupBox)"
    draft            = $true
    prerelease       = $false
}
...
$a2 = Invoke-RestMethod -Uri "https://uploads.github.com/repos/$owner/$repo/releases/$($release.id)/assets?name=Crea_Cout_3D_Setup_1.8.3.0.exe" -Method POST -Headers $uploadHeaders -Body $setupBytes
