...
$setupPath = "D:\projet_3d\logiciel d'impression 3d\Installer\Crea_Cout_3D_Setup_1.8.1.0.exe"
...
Write-Host "1. Creation release v1.8.1..."
$releaseObj = @{
    tag_name         = "v1.8.1"
    target_commitish = "master"
    name             = "v1.8.1 - Import G-code, mode serie, reinitialisation mot de passe"
    body             = "### Nouvelles fonctionnalites`n`n- Import G-code Bambu/OrcaSlicer dans l'onglet 3MF (temps et poids extraits automatiquement)`n- Mode serie : quantite configurable, affichage prix unitaire + total`n- Reinitialisation de mot de passe par email (code a 6 chiffres, 15 min)`n- Cache de session chiffre (DPAPI) pour plus de securite"
    draft            = $true
    prerelease       = $false
}
...
$a2 = Invoke-RestMethod -Uri "https://uploads.github.com/repos/$owner/$repo/releases/$($release.id)/assets?name=Crea_Cout_3D_Setup_1.8.1.0.exe" -Method POST -Headers $uploadHeaders -Body $setupBytes
