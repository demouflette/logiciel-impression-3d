...
$setupPath = "D:\projet_3d\logiciel d'impression 3d\Installer\Crea_Cout_3D_Setup_1.8.4.0.exe"
...
Write-Host "1. Creation release v1.8.4..."
$releaseObj = @{
    tag_name         = "v1.8.4"
    target_commitish = "master"
    name             = "v1.8.4 - Correction AMS affichage DPI"
    body             = "### Correction`n`n- Correction : Nombre de couleurs masque apres avoir coche Utiliser AMS (probleme de mise a l'echelle DPI)"
    draft            = $true
    prerelease       = $false
}
...
$a2 = Invoke-RestMethod -Uri "https://uploads.github.com/repos/$owner/$repo/releases/$($release.id)/assets?name=Crea_Cout_3D_Setup_1.8.4.0.exe" -Method POST -Headers $uploadHeaders -Body $setupBytes
